...
app.get("/product/:id", function (req, res) {
  ...
  database.query(
    "SELECT Meubles.nom, Meubles.descriptif, Meubles.photo, Meubles.prix, Meubles.stock, Meubles.dimension, Categories.nom AS categorie, Matieres.nom AS matiere FROM Meubles INNER JOIN Categories ON Meubles.categorie_id = Categories.id INNER JOIN Matieres ON Meubles.matiere_id = Matieres.id WHERE Meubles.id = ?",
    [productId],
    (err, rows, fields) => {
      if (err) {
        console.log("Erreur dans la requête", err);
...
app.post("/admin", (req, res) => {
  ...
  database.query(addMeubles, [
    meubleAjoute.nom,
    meubleAjoute.categorie_id,
    meubleAjoute.descriptif,
    meubleAjoute.prix,
    meubleAjoute.dimension,
    meubleAjoute.vendeur_id,
    meubleAjoute.acheteur_id,
    meubleAjoute.matiere_id,
    meubleAjoute.photo,
    meubleAjoute.couleur_id,
    meubleAjoute.stock,
  ]);
  res.status(201).json({
    message: "Objet créé !",
  ...
  database.query(
    ...
    (err, result) => {
      ...
      database.query(
        addUtilisateur,
        [
          utilisateurAjoute.nom,
          utilisateurAjoute.mail,
          motdepasseId,
          utilisateurAjoute.telephone,
        ],
        (err, result) => {
...
app.put("/meubles/:id", (req, res) => {
  ...
  let paramQuery = [
    ...
    couleur_id,
    id,
  ]; //Bien mettre les paramètres dans ce tableau dans l'ordre des points d'interrogation de la requête
  database.query(
    "UPDATE Meubles SET  nom = ?, categorie_id = ?, descriptif = ?, prix = ?, dimension = ?, vendeur_id = ?, acheteur_id = ?, matiere_id = ?, couleur_id = ?  WHERE id = ?",
    paramQuery,
    (err, result) => {
      if (err) {
        res.json({
...
app.get("/product/:id", function (req, res) {
  ...
  database.query(
    "SELECT Meubles.id, Meubles.nom, Meubles.descriptif, Meubles.photo, Meubles.prix, Meubles.stock, Meubles.dimension, Categories.nom AS categorie, Matieres.nom AS matiere FROM Meubles INNER JOIN Categories ON Meubles.categorie_id = Categories.id INNER JOIN Matieres ON Meubles.matiere_id = Matieres.id WHERE Meubles.id = ?",
    [productId],
    (err, rows, fields) => {
      if (err) {
        console.log("Erreur dans la requête", err);
...
app.get("/admin/categorie", function (req, res) {
  database.query(
    "SELECT Categories.nom, Categories.id FROM Categories",
    (err, rows) => {
      if (err) {
        console.log("erreur dans la requête", err);
        res.status(500).send("erreur interne du serveur");
...
app.get("/searchbar", function (req, res) {
  //suppression /meuble pas de page meuble >> Lise
  ...
  if (
    ...
    database.query(
      "SELECT Meubles.id, Meubles.nom, Meubles.descriptif,Meubles.photo, Meubles.prix FROM Meubles",
      (err, rows, fields) => {
        if (err) {
          console.log("erreur dans la requête", err);
          res.status(500).send("erreur interne du serveur");
  ...
  //On gère au cas par cas les paramètres passés. Pour l'instant la fonction ne gère pas plusieurs paramètres en meme temps
    ...
    database.query(
      "SELECT Meubles.nom, Meubles.descriptif,Meubles.photo, Meubles.prix FROM Meubles INNER JOIN Couleurs ON Meubles.couleur_id = Couleurs.id WHERE Couleurs.nom = ?",
      [couleur],
      (err, rows, fields) => {
        if (err) {
          console.log(
            "Les meubles de cette couleur n'ont pas été trouvés",
            err.message
          );
          res.status(500).send("erreur interne du serveur");
          return;
    ...
    database.query(
      "SELECT Meubles.nom, Meubles.descriptif,Meubles.photo, Meubles.prix FROM Meubles INNER JOIN Categories ON Meubles.categorie_id = Categories.id WHERE Categories.nom = ?",
      [categorie],
      (err, rows, fields) => {
        if (err) {
          console.log(
            "Les meubles de cette categorie n'ont pas été trouvés",
            err.message
          );
          res.status(500).send("erreur interne du serveur");
          return;
    ...
    database.query(
      "SELECT Meubles.nom, Meubles.descriptif,Meubles.photo, Meubles.prix FROM Meubles INNER JOIN Matieres ON Meubles.matiere_id = Matieres.id WHERE Matieres.nom = ?",
      [matiere],
      (err, rows, fields) => {
        if (err) {
          console.log(
    ...
    database.query(
      "SELECT Meubles.nom, Meubles.descriptif,Meubles.photo, Meubles.prix FROM Meubles WHERE id = ?",
      [id],
      (err, rows, fields) => {
        if (err) {
...
app.get("/meublesenstock/:id", (req, res) => {
  ...
  database.query(
    "SELECT * FROM Meubles WHERE stock = 1 AND id = ?",
    [id],
    (err, rows, fields) => {
      if (err) {
        console.log("Le meuble n'a pas été trouvé", err.message);
